...
AY-3-8910 -- голос ZX Spectrum 128K. General Instrument спроектировала его в 1978 году как программируемый генератор звука (PSG, Programmable Sound Generator) -- единственный чип, способный воспроизводить музыку и звуковые эффекты без выделенного звукового процессора. К тому моменту, когда Amstrad установила его в Spectrum 128K в 1986 году, он уже был проверенной рабочей лошадкой: Intellivision, MSX, Atari ST и десятки аркадных автоматов использовали его или его pin-совместимые варианты (YM2149 в Atari ST, AY-3-8912 с меньшим количеством портов ввода-вывода).
...
**Малый барабан:** шумовой канал + быстрое затухание огибающей.
...
**Бас-бочка:** быстрый свип тона вниз. Установи канал C на низкий период тона, затем увеличивай период в течение нескольких кадров:
...
Орнамент -- это таблица покадровых смещений, применяемых к высоте тона или громкости ноты. Орнаменты вдыхают жизнь в иначе статичные прямоугольные волны: вибрато, глиссандо, тремоло, огибающие атаки/затухания -- всё достигается табличным поиском.
...
Движок проигрывателя применяет смещение орнамента к значению периода базовой ноты каждый кадр, создавая плавную модуляцию.
...
На плате NedoPC TurboSound (наиболее распространённый современный дизайн) выбор чипа работает через порт $FFFD:
...
Адаптация одно-AY движка к TurboSound проста. Твой обработчик прерываний становится:
...
Цикл записи регистров записывает все 14 регистров из буфера в ОЗУ. Для двух чипов ты поддерживаешь два 14-байтных буфера и выгружаешь их последовательно. Общая стоимость: около 28 инструкций OUT, примерно 400 тактов (T-state).
...
| Канал | Назначение | Панорама |
|-------|-----------|----------|
| Чип 0, A | Басовая линия | Центр |
| Чип 0, B | Ритм-гитара / пэд | Лево |
| Чип 0, C | Ведущая мелодия | Центр |
| Чип 1, A | Гармония / контрмелодия | Право |
| Чип 1, B | Арпеджио-аккорд | Лево |
| Чип 1, C | Перкуссия: бочка + малый | Центр |
| Чип 2, A | Хай-хэт / тарелка | Право |
| Чип 2, B | Звуковые эффекты (резерв) | Центр |
| Чип 2, C | Эмбиент / пэд-слои | Лево |
...
1. Уменьшает счётчик длительности текущей ноты
2. Если ноль -- переходит к следующей ноте в паттерне
3. Применяет орнаменты и эффекты (арпеджио, вибрато, глиссандо) к каждому каналу
4. Вычисляет итоговый период и громкость для каждого канала
5. Записывает все 14 регистров AY
...
Играм нужны звуковые эффекты, а звуковым эффектам нужны каналы. Стандартный подход -- **приоритетная кража каналов**: когда срабатывает звуковой эффект, он временно захватывает канал у музыкального движка, а затем освобождает его, когда эффект заканчивается.
...
Звуковые эффекты определяются как таблицы покадровых значений регистров. Вот четыре классических игровых звука:
...
1. **Двухшаговый доступ к порту**: выбор регистра через $FFFD, затем запись данных через $BFFD.
2. **Конфигурация микшера**: $3E включает только тон A (двоичное `111 110` -- помни, 0 = ВКЛ).
3. **Таблица нот**: предрассчитанные значения периодов для каждой высоты тона.
4. **Тайминг на HALT**: каждый `HALT` ждёт одно прерывание, давая разрешение тайминга 50 Гц.
...
**Упражнение 3: Набор ударных.** Реализуй четыре звука ударных (бочка, малый, хай-хэт, крэш) как процедурные таблицы SFX. Напиши простой паттерн ударных, воспроизводящий их последовательно. Интегрируй с мелодией из Упражнения 2.
...
В следующей главе мы применим это на практике: цифровые барабанные сэмплы, смешанные с воспроизведением AY, покадрово точная синхронизация музыки и визуальных эффектов, и скриптовые движки, связывающие демо воедино.

---

> **Источники:** Dark "GS Sound System" (Spectrum Expert #01, 1997); Dark "Music" (Spectrum Expert #02, 1998); Shiru "Beeper 20XX" (Hype 2016); Rain file_id.diz; Info Guide #14 ASC Sound Master docs (ZXArt 2024); Vortex Tracker II documentation
