...
Это компилируется в Z80-ассемблер, собирается в бинарник и работает на реальном или эмулируемом железе. Самодостаточный инструментарий --- компилятор, ассемблер, эмулятор, REPL, удалённый запуск --- означает отсутствие внешних зависимостей.
...
**Эмулятор.** `mze` достигает 100% покрытия инструкций Z80. ИИ сгенерировал начальную реализацию каждой инструкции по руководству Z80; тестовый набор (тоже сгенерированный ИИ) поймал крайние случаи --- поведение флагов при переполнении, флаг полупереноса в DAA, тайминги прерываний.
...
MinZ не мог бы существовать без помощи ИИ. Огромный объём систематического кода --- кодировщик инструкций, эмулятор, стандартная библиотека, peephole-паттерны --- занял бы у одного разработчика годы ручной работы. С помощью ИИ MinZ прошёл путь от концепции до v0.18.0 примерно за 18 месяцев.

Но *интересные* особенности MinZ --- TSMC, трансформация лямбд в функции с нулевыми накладными расходами, UFCS-диспетчеризация методов --- это человеческие изобретения. ИИ их реализовал, но не придумал.
...
Это корректно для первых двух третей экрана, но ломается на границе между второй и третьей третью. Экранная память делится на три части по 2 048 байт, и переход от второй к третьей трети требует добавления $08 к H, а не вычитания. ИИ сгенерировал код, который правильно обрабатывает общий случай, но ошибается в граничном.

В статье Introspec представлена версия RST7, использующая подход с двойным счётчиком, которая корректно обрабатывает все границы за 2 343 T-state для полного прохода по экрану. Наивная версия --- та, которую большинство людей пишут с первой попытки, и которую аппроксимирует ИИ --- стоит 5 922 T-state. Разрыв между «работает» и «работает хорошо» --- в 2,5 раза.
...
Каждый инструмент, поднимающий уровень абстракции на ограниченном железе, следует одной траектории. BASIC: доступный, медленный, прожорливый к памяти. HiSoft C: быстрее, более структурированный, но ограниченный. Макроассемблеры и кросс-разработка: тот же уровень абстракции, меньше трения.

Разработка с помощью ИИ --- последний шаг. Она не меняет уровень абстракции результата --- ты по-прежнему пишешь Z80-ассемблер, и Z80 по-прежнему выполняет те же инструкции на тех же скоростях. Что она меняет --- скорость ввода: как быстро ты переходишь от идеи к работающему (пусть и неоптимизированному) коду.

Демосцена ZX Spectrum была ограничена не только аппаратными возможностями машины, но и *человеческой стоимостью* её программирования. Писать Z80-ассемблер --- медленно. Отлаживать его --- ещё медленнее. Изучение трюков занимает годы. Помощь ИИ снижает порог входа, не опуская потолок. Эксперты по-прежнему будут писать более качественные внутренние циклы, чем любой ИИ. Но больше людей дойдут до точки, где смогут понять, *почему* эти внутренние циклы лучше.

Это хорошо для сцены. Демосцена процветает на участии. Если сочетание задокументированных техник и инструментов с помощью ИИ привлечёт новых творцов в сцену ZX Spectrum, компромисс того стоит.

Introspec прав, что ИИ не знает Z80 --- не так, как знает он. Но ИИ и не нужно знать Z80 так, как Introspec. Ему нужно знать его достаточно хорошо, чтобы помочь новичку начать, совершать меньше ошибок и дойти до точки, где он сможет изучить глубокие трюки из статей Introspec самостоятельно.

Ни магия, ни бесполезность. Инструмент. Как HiSoft C, но другой.
...
4. **Эксперимент с MinZ.** Установи инструментарий MinZ (`mza`, `mze`). Напиши простую программу на MinZ --- заполнение экрана, чтение клавиатуры, прыгающий пиксель. Сравни сгенерированный ассемблер с тем, что ты написал бы вручную. Где вывод MinZ хорош? Где расточителен?

5. **Построй что-нибудь.** Выбери эффект из одной из предыдущих глав. Используй помощь ИИ для написания первого черновика. Итерируй до работающего результата. Профилируй его. Оптимизируй внутренний цикл вручную. Задокументируй каждый шаг. Ты только что прошёл через тот самый рабочий процесс, который описывает вся эта глава.
...
> **Источники:** Обзор HiSoft C (Spectrum Expert #02, 1998); Introspec «Technical Analysis of Illusion» (Hype, 2017); Introspec «DOWN_HL» (Hype, 2020); Introspec «GO WEST Parts 1-2» (Hype, 2015)
