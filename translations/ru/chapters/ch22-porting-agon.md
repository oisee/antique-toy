...
В этом настоящий урок портирования. У каждой платформы есть ограничения, которые подталкивают к лучшей инженерии, но ограничения разные.

### Spectrum заставляет тебя быть эффективным

Когда твой бюджет кадра --- 71 680 T-state, а один только спрайтовый движок потребляет 12 000 из них, ты учишься считать такты. Учишься использовать `INC L` вместо `INC HL`. Учишься развёртывать циклы. Учишься предвычислять всё, что можно. Учишься думать о раскладке данных --- хранить связанные данные в непрерывной памяти, выравнивать буферы по границам страниц, располагать поля структур так, чтобы наиболее часто используемые имели наименьшие смещения.

Эта дисциплина переносится на любую платформу, с которой ты когда-либо будешь работать. Spectrum учит тебя, что «эффективность» на самом деле означает на уровне инструкций, и это осознание тебя уже не покидает. Даже на Agon, где тактов в избытке, привычка писать плотные внутренние циклы стоит того.

### Spectrum заставляет тебя быть креативным

Конфликт атрибутов. Чересстрочная раскладка экранной памяти. 6 912-байтный фреймбуфер, который выглядит так, будто его проектировал комитет, который ни разу не собирался. Этих ограничений нет на Agon, и их отсутствие убирает творческое давление, породившее некоторые из самых характерных визуальных техник Spectrum.

Мультиколор, трюк с 4-фазным цветом, эффекты только на атрибутах, переключение третей экрана для двойной буферизации --- это решения проблем, которых на Agon не существует. Ты их не изобретёшь, если программируешь только Agon. Ограничения Spectrum --- это школа творческого решения задач.

### Agon заставляет тебя думать об архитектуре

Когда память больше не дефицитна, когда такты больше не впритык, оставшиеся вызовы --- архитектурные. Как структурировать игровой цикл, который общается с асинхронным сопроцессором? Как рассчитывать команды VDP так, чтобы спрайты двигались плавно? Как управлять потоком данных от процессора к VDP через последовательный канал?

Agon учит тебя системному проектированию: разделению ответственности, управлению задержкой, структурированию командных протоколов и построению слоёв абстракции. Это навыки программной инженерии, которые Spectrum с его философией «пиши напрямую в железо» не подчёркивает.

### Agon заставляет тебя заботиться о данных

На Spectrum спрайт 16x16 --- это 32 байта пиксельных данных плюс 32 байта маски. На Agon тот же спрайт в формате 8bpp RGBA --- 1 024 байта. Умножь на 64 спрайта по 4 кадра анимации --- и ты получишь 256 КБ одних только данных спрайтов. Даже с 512 КБ ты начинаешь думать о пайплайнах ресурсов: как конвертировать, оптимизировать и управлять большими объёмами графических данных.

Agon делает тебя лучше как *создателя инструментов* --- написание конвертеров, оптимизаторов, скриптов пайплайна ресурсов. Spectrum делает тебя лучше как *оптимизатора* самого кода. Оба навыка важны.
