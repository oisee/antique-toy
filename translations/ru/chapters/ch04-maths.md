...
Отсечение задних граней в главе 5 передаёт первый операнд в A, а не в B. Тонкая обёртка позволяет не перестраивать вызывающий код:
...
|---------|-------|--------|----------|-------|
| `mulu112` (беззнаковая) | B, C | A:C (16-бит) | 196–204 | Глава 4, умножение сдвигом и сложением |
| `mulu_fast` (таблица квадратов) | B, C | HL (16-бит) | ~61 | Нужна таблица 512 байт; ошибка округления |
| `mul_signed` | B, C (знаковые) | HL (знаковый 16-бит) | ~240–260 | Обработка знака добавляет ~40–60T |
| `mul_signed_c` | A, C (знаковые) | HL (знаковый 16-бит) | ~250–270 | Обёртка для отсечения задних граней |
...
### На шаг дальше: техника коррекции беззнакового результата

Подход с 2×abs выше понятен и корректен, но существует более элегантный метод. Ключевое математическое тождество: для N-битного числа в дополнительном коде `a_знаковое = a_беззнаковое − 2^N × бит_знака`. Для умножения 8×8:

```
a_s × b_s = a_u × b_u − 256 × знак(a) × b_u − 256 × знак(b) × a_u
```

(Член `+65536 × знак(a) × знак(b)` переполняет 16-битный результат и исчезает.)

На практике: **выполни беззнаковое умножение, затем скорректируй старший байт.** Если A был отрицательным — вычти B из старшего байта. Если B был отрицательным — вычти A из старшего байта. Никаких абсолютных значений, никакой условной инверсии результата.

На чистом Z80 экономия по сравнению с 2×abs скромная --- цикл сдвигов и сложений всё равно доминирует. Но на Z80N, где `MUL DE` выполняет беззнаковое умножение 8×8 одной инструкцией, техника коррекции сжимает знаковое умножение до ~70 T-states и 16 байт:

```z80
; Знаковое умножение 16x8 на Z80N (Ped7g)
; Вход:  DE = x (16 бит, знаковое), L = y (8 бит, знаковое)
; Выход: AL = знаковый 16-битный результат
; Стоимость: ~70T, 16 байт. Требует инструкцию MUL Z80N.

    xor  a
    bit  7, l           ; y отрицательный?
    jr   z, .y_pos
    sub  e              ; компенсация: старший байт −= low(x)
.y_pos:
    ld   h, d           ; H = x_старший, L = y
    ld   d, l           ; D = y,         E = x_младший
    mul  de             ; DE = x_младший * y (беззнаковое)
    add  a, d           ; накопить старший байт
    ex   de, hl         ; L = младший байт результата
    mul  de             ; DE = x_старший * y (беззнаковое)
    add  a, e           ; итоговый старший байт → A:L
    ret
```

Тот же принцип работает для чистого Z80 --- сохрани исходные операнды перед циклом сдвигов и сложений, затем примени два условных вычитания к старшему байту. Код немного короче, чем 2×abs+neg, и обходится без PUSH/POP для флага знака.

> **Благодарность:** Техника коррекции и реализация Z80N — Ped7g. Варианты для чистого Z80 — base и busy (SinDiKat).
...
Алгоритм Брезенхэма шагает вдоль главной оси по одному пикселю за раз, поддерживая аккумулятор ошибки для шагов по второстепенной оси. На Spectrum «поставить пиксель» — дорогая операция: чересстрочная раскладка экрана означает, что вычисление адреса байта и битовой позиции стоит реальных тактов. Подпрограмма ПЗУ тратит более 1000 тактов на пиксель. Даже ручной оптимизированный цикл Брезенхэма стоит ~80 тактов на пиксель.
...
| Patrik Rak CMWC (alt) | ~35 + 10 таблица | ~180 | Посредственное | ~2^66 | Паттерны видны в крупном масштабе |
...
![Вывод ГПСЧ --- случайные цвета атрибутов заполняют экран, выявляя статистическое качество генератора](../../build/screenshots/ch04_prng.png)
...
> **Источники:** Dark / X-Trade, «Programming Algorithms» (Spectrum Expert #01, 1997); Gogin, коллекция ГПСЧ и оценка качества; Patrik Rak (Raxoft), CMWC-генератор; Ped7g (Peter Helcmanovsky), выявление пробела в знаковой арифметике и ревью
