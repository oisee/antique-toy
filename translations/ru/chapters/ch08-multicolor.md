...
Демосцена всегда была о том, чтобы выжимать из оборудования больше заложенного. Но есть различие --- иногда упускаемое, иногда намеренное --- между выжиманием для десятисекундного демо и для играбельной игры.

Демо --- перформанс. Они запускаются один раз, впечатляют, заканчиваются. Нет обработки ввода. Нет определения коллизий. Нет состояния, сохраняемого от кадра к кадру (кроме того, что нужно эффекту внутренне). Демо может тратить 100% тактов на визуальное зрелище, потому что зрелище --- всё, что нужно произвести.

Игры --- инженерия. Они должны читать клавиатуру, обновлять позиции сущностей, проверять коллизии, воспроизводить звук, управлять игровым состоянием и рендерить экран --- каждый кадр, вечно, оставаясь отзывчивыми к игроку. Каждая из этих подсистем конкурирует за один и тот же бюджет в 70 000 тактов. Техника, работающая в демо, но потребляющая 90% процессора, бесполезна для игр.

Достижение DenisGrachev'а --- в решении инженерной проблемы. Мультиколор был известен. Визуальный потенциал был известен. Инструкционный паттерн LDPUSH, пожалуй, был неявно заложен в долгой истории стековых трюков вывода на демосцене (Глава 3). Что не было известно --- как уместить мультиколорный рендеринг, тайловые движки, наложение спрайтов, двойную буферизацию, звук, ввод и игровую логику в один бюджет кадра. Двухкадровая архитектура, объединённый буфер кода-как-данных, компромисс звука на 25 Гц, тщательное распределение бюджета --- это решения игрового движка, а не демо-трюки.

Движок Ringo движется дальше в другом направлении. Используя аппаратное переключение двух экранов вместо синхронизированной с лучом смены атрибутов, он обменивает цветовое разрешение (64x48 вместо 192x128 пиксельной сетки GLUF) на радикально более дешёвый путь рендеринга. Процессорные накладные расходы переключения экранов --- горстка инструкций `OUT` за кадр. Что ты получаешь --- бюджет тактов. На что тратишь --- на игровую логику.

Оба движка представляют философию: техники демосцены --- не музейные экспонаты. Это инженерные инструменты, применимые к реальным продуктам, если ты готов проделать работу по превращению их в практичные. Мультиколорные игры DenisGrachev'а не выглядят как игры ZX Spectrum. Они выглядят как игры, которые случайно работают на ZX Spectrum. Это различие --- вся суть.
