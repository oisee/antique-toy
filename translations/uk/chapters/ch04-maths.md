...
### Крок далі: техніка корекції беззнакового результату

Підхід із 2×abs вище зрозумілий і коректний, але існує елегантніший метод. Ключова математична тотожність: для N-бітного числа у доповняльному коді `a_знакове = a_беззнакове − 2^N × біт_знаку`. Для множення 8×8:

```
a_s × b_s = a_u × b_u − 256 × знак(a) × b_u − 256 × знак(b) × a_u
```

(Член `+65536 × знак(a) × знак(b)` переповнює 16-бітний результат і зникає.)

На практиці: **виконай беззнакове множення, потім скоригуй старший байт.** Якщо A був від'ємним — відніми B від старшого байта. Якщо B був від'ємним — відніми A від старшого байта. Жодних абсолютних значень, жодної умовної інверсії результату.

На чистому Z80 економія порівняно з 2×abs скромна --- цикл зсувів і додавань усе одно домінує. Але на Z80N, де `MUL DE` виконує беззнакове множення 8×8 однією інструкцією, техніка корекції стискає знакове множення до ~70 T-states і 16 байт:

```z80
; Знакове множення 16x8 на Z80N (Ped7g)
; Вхід:  DE = x (16 біт, знакове), L = y (8 біт, знакове)
; Вихід: AL = знаковий 16-бітний результат
; Вартість: ~70T, 16 байт. Потребує інструкцію MUL Z80N.

    xor  a
    bit  7, l           ; y від'ємний?
    jr   z, .y_pos
    sub  e              ; компенсація: старший байт −= low(x)
.y_pos:
    ld   h, d           ; H = x_старший, L = y
    ld   d, l           ; D = y,         E = x_молодший
    mul  de             ; DE = x_молодший * y (беззнакове)
    add  a, d           ; накопичити старший байт
    ex   de, hl         ; L = молодший байт результату
    mul  de             ; DE = x_старший * y (беззнакове)
    add  a, e           ; підсумковий старший байт → A:L
    ret
```

Той самий принцип працює для чистого Z80 --- збережи початкові операнди перед циклом зсувів і додавань, потім застосуй два умовних віднімання до старшого байта. Код дещо коротший за 2×abs+neg і обходиться без PUSH/POP для прапора знаку.

> **Подяка:** Техніка корекції та реалізація Z80N — Ped7g. Варіанти для чистого Z80 — base і busy (SinDiKat).
