...
Introspec pushed this further with what he called pseudo-chunky rendering. Rather than treating all 32x24 cells as uniform pixels, the code uses variable-size blocks. In the centre of the screen, several attribute cells might share the same colour, creating effectively larger "pixels" -- 16x16 or even 24x24 areas of uniform colour. Toward the edges, each 8x8 cell gets its own value. The result exploits a perceptual trick: the eye accepts large, blocky pixels in the middle because that is where the tunnel mouth is, where depth naturally destroys detail. The peripheral vision picks up the finer resolution at the edges, creating an impression of higher fidelity than the data actually contains.
